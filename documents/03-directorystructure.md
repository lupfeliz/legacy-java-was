...
- 비즈니스 로직 파일(또는 메소드) 및 jsp 파일명은 분류코드 일부(또는 전체) 와 약속어휘(어두, 어미) 조합으로 이루어진다.
...
| `~ Entity`     | DTO 및 엔티티          | Inner-Class 로 dto 작성 가능                 |
...
- URL 등록의 경우 단순하게 `대분류`/`코드명` 으로 작성하면 된다  (예시 : `/smp/smp01001s01`)

- REST 요청 경로 명명규칙

  REST 요청 경로의 경우 반드시 `/api/` 로 시작하며, 이후 경로 규칙은 일반 URL 등록규칙과 같다 (예시 : `/api/smp/smp01001a01`)
